...
images = home + '/cards/imagesFolders/'
masks = home + '/cards/masksFolders/'
backgrounds = os.getcwd() + '/cards/backgrounds/'
bgImages = home+ '/cards/backgroundNoise/images/'
bgMasks = home+ '/cards/backgroundNoise/masks/'

cardDict = cDict()


def imgMaskCreate():
    pbar = tqdm.tqdm(total=11933)
    for fl in os.scandir(baseImages):

        picDir = baseImages + fl.name
        fldImgName = images + os.path.splitext(fl.name)[0]

        if not os.path.isdir(fldImgName):
            os.makedirs(fldImgName)
            print('Created the RGB image folder')

        img = cv2.imread(picDir)
        os.chdir(fldImgName)
        cv2.imwrite(str(fl.name), img)

        msk = cv2.rectangle(img, (0, 0), (421, 614), (255, 255, 255), 15)
        msk = cv2.rectangle(mask, (10, 10), (411, 604), (0, 0, 0), -1)

        fldMskName = masks + os.path.splitext(fl.name)[0]

        if not os.path.isdir(fldMskName):
            os.makedirs(fldMskName)
            print('Created the mask image folder')

        os.chdir(fldMskName)
        cv2.imwrite(str(fl.name), msk)
        pbar.update(1)

    pbar.close()

def get_img_and_mask(imgDir, mskPth):

    img = cv2.imread(imgDir)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    mask = cv2.imread(mskPth)
...
def add_obj(img_comp, mask_comp, img, mask, x, y, idx):
    ...
    if x >= 0 and y >= 0:
        ...
        img_comp[y:y + h_part, x:x + w_part, :] = img_comp[y:y + h_part, x:x + w_part, :] * ~mask_rgb_b[0:h_part,
                    0:w_part, :] + (img * mask_rgb_b)[0:h_part, 0:w_part, :]
        ...
        img_comp[0:0 + h_part, 0:0 + w_part, :] = img_comp[0:0 + h_part, 0:0 + w_part, :] * ~mask_rgb_b[h - h_part:h,
                    w - w_part:w, :] + (img * mask_rgb_b)[h - h_part:h, w - w_part:w, :]

        mask_comp[0:0 + h_part, 0:0 + w_part] = mask_comp[0:0 + h_part, 0:0 + w_part] * ~mask_b[h - h_part:h,
                    w - w_part:w] + (idx * mask_b)[h - h_part:h, w - w_part:w]
        ...
        img_comp[y:y + h_part, 0:0 + w_part, :] = img_comp[y:y + h_part, 0:0 + w_part, :] * ~mask_rgb_b[0:h_part,
                    w - w_part:w, :] + (img * mask_rgb_b)[0:h_part, w - w_part:w, :]

        mask_comp[y:y + h_part, 0:0 + w_part] = mask_comp[y:y + h_part, 0:0 + w_part] * ~mask_b[0:h_part,
                    w - w_part:w] + (idx * mask_b)[0:h_part, w - w_part:w]
        ...
        img_comp[0:0 + h_part, x:x + w_part, :] = img_comp[0:0 + h_part, x:x + w_part, :] * ~mask_rgb_b[h - h_part:h,
                    0:w_part, :] + (img * mask_rgb_b)[h - h_part:h, 0:w_part, :]

        mask_comp[0:0 + h_part, x:x + w_part] = mask_comp[0:0 + h_part, x:x + w_part] * ~mask_b[h - h_part:h,
                    0:w_part] + (idx * mask_b)[h - h_part:h, 0:w_part]
...
def create_composition(img_comp_bg, max_objs=15, overlap_degree=0.2, max_attempts_per_obj=10):
    ...
    for _ in range(1, num_objs):
        ...
        for _ in range(max_attempts_per_obj):
            ...
            img, mask = resize_transform_obj(img, mask, longest_min, longest_max, transforms=transforms_obj)

            if i == 1:
                img_comp, mask_comp, mask_added = add_obj(img_comp, mask_comp, img, mask, x, y, i)
...
img, mask = get_img_and_mask(images+'/7/7.jpg', masks+'/7/7.jpg')
...
#
# filename = ET.SubElement(root,'filename').text = fl.name + '.jpg'
#
# object = ET.SubElement(root, 'object')
#
# ET.SubElement(object, 'name').text = fl.name
#
# bndbox = ET.SubElement(object, 'bndbox')
# ET.SubElement(bndbox, 'xmin').text = '10'
# ET.SubElement(bndbox, 'xmax').text = '411'
# ET.SubElement(bndbox, 'ymin').text = '10'
# ET.SubElement(bndbox, 'ymax').text = '604'
#
# ET.SubElement(object, 'difficult').text = '0'
#
# tree = ET.ElementTree(root)
# tree.write('text.xml', encoding='utf8')